"""
European Call option priced under the skew-t modified Gosset formula.
...
The 1-D skewness parameter ``alpha_skew`` for each underlying is derived
from the multivariate AC skew-t parameters (scale matrix Omega and slant
vector omega) via::

    delta_i  = h_i / sqrt(Omega_ii),          h = Omega @ omega / sqrt(1 + omega^T Omega omega)
    alpha_i  = delta_i / sqrt(1 - delta_i^2)

This is the marginal slant of the i-th component of the multivariate skew-t
in the standardized-residual representation.
"""
...
class SkewTGossetCall(GossetCall):
    """
    European Call option priced under the Gosset formula with a 1-D skew-t density.

    Identical to :class:`~src.options.gosset_call.GossetCall` except that the
    ...
    def __init__(
        ...
        """
        Initialize a SkewTGossetCall option.
    ...
    def _compute_price_with_spot(self, spot: float) -> float:
        """Compute price with a different spot price (for numerical delta/gamma)."""
