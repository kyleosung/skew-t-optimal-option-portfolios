"""
European Call option priced under the Black-Scholes model.
"""
...
class EuropeanCall(Option):
    ...
    def price(self) -> float:
        ...
        price = S * np.exp(-q * tau) * norm.cdf(self.d1) - K * np.exp(
            -r * tau
        ) * norm.cdf(self.d2)
        return price.item()
    ...
    def delta(self) -> float:
        ...
        q = self.dividend_yield
        tau = self.time_to_maturity
        return (np.exp(-q * tau) * norm.cdf(self.d1)).item()
    ...
    def theta(self) -> float:
        ...
        term1 = -np.exp(-q * tau) * (S * norm.pdf(d1) * sigma) / (2 * np.sqrt(tau))
    ...
    @cached_property
    def rho(self) -> float:
        """Black-Scholes Rho (per 1.0 change in interest rate)."""
        K = self.strike
        r = self.market_env.annual_risk_free_rate
        tau = self.time_to_maturity
        d2 = self.d2
        # Rho per 1.0 change in interest rate
        return (K * tau * np.exp(-r * tau) * norm.cdf(d2)).item()
